...
-- 6. ТРИГГЕРЫ
-- ===================================
-- Триггер для обновления total_spent и orders_count при изменении статуса на 'DELIVERED'
...
WITH RECURSIVE bfs_search AS (
    ...
    FROM city c
    WHERE c.city_name = 'Начальный Город' -- <<< --- ИЗМЕНИТЬ ЗДЕСЬ
...
SELECT
    city_name AS "Конечный Город",
    level - 1 AS "Кол-во Пересадок (BFS)",
    total_distance AS "Общее Расстояние (км)",
    path_name AS "Путь (Города)"
FROM bfs_search
